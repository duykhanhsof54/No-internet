...
bool LoadMedia()
{
	...
	gMusic = Mix_LoadMUS("sound/sound_bkgr_audio.wav");
    if (gMusic == nullptr)
	{
	...
	gMenuMusic = Mix_LoadMUS("sound/sound_menu_audio.wav");
	...
	gClick = Mix_LoadWAV("sound/sound_mouse_click.wav");
	if (gClick == nullptr)
	{
	...
	gJump = Mix_LoadWAV("sound/sound_jump_sound.wav");
	if (gJump == nullptr)
	{
	...
	gLose = Mix_LoadWAV("sound/sound_lose_sound.wav");
	if (gLose == nullptr)
	{
	...
	else
	{
		...
		else
		{
			...
			if (!gMenuTexture.LoadFromFile("imgs/menu.png", gRenderer))
			{
			...
			if (!gGroundTexture.LoadFromFile("imgs/ground.png", gRenderer))
			{
				std::cout << "Failed to load ground image" << std::endl;
				success = false;
			}

			if (!gCharacterTexture.LoadFromFile("imgs/char.png", gRenderer))
			{
			...
			if (!gLoseTexture.LoadFromFile("imgs/lose.png", gRenderer))
			{
